package com.AntonSibgatulin.Players.beast;
...
public class BeastModel extends EntityBreath implements IBreath, Cloneable {

	public BeastModel(JSONObject jsonObject) {
	...
	public void fire() {
		...
		if (key == 4 || key == 9) {
			/*
			 * final int size = MapGeneration.SIZE_FIGHT; final int speed_blade
			 * = size / 2; double w = player.side == 1 ? player.position.w :
			 * -size;
			 * 
			 * // final Square square = new Square(player.position.x + size * 2
			 ...
			 * (player.position.h / 2 - size / 2), -3 * speed_blade *
			 * player.side, 0); player.fights.add(shot); int end = size * 6; int
			 * col = 14;
			 */
			final int size = MapGeneration.SIZE;// (int)
			// (MapGeneration.SIZE_FIGHT / 2
			...
			mainTimer = new Timer(time, col, new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					// square.x -= speed_blade;
					/*
					 * if (player.side == 1) { if (shot.dtx >= player.position.w
					 * + 2 * size) { shot.mx = 3 * speed_blade * player.side; }
					 * if (shot.dtx <= -2 * size) { shot.mx = -3 * speed_blade *
					 * player.side; } } else { if (shot.dtx >= player.position.w
					 * + 2 * size) { shot.mx = 3 * speed_blade * -player.side; }
					 * if (shot.dtx <= -2 * size) { shot.mx = -3 * speed_blade *
					 * -player.side; } } // shot.dtx *= player.side;
					 * shot.update();
					 */
				}
		...
		/*
		 * if (key == 9) { int size = MapGeneration.SIZE_FIGHT; final int
