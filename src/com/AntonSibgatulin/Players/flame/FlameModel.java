package com.AntonSibgatulin.Players.flame;
...
public class FlameModel extends EntityBreath implements IBreath, Cloneable {

	public FlameModel(JSONObject jsonObject) {
	...
	public void fire() {
		...
		if (key == 1) {
			/*
			 * final int size = (int) (MapGeneration.SIZE_FIGHT / 2 * 1.8); int
			 * s = MapGeneration.SIZE_FIGHT;
			 * 
			 * final Shot shot = new Shot(player.position, s, s, player.side *
			 * (player.position.w / 2 - size / 2), (player.position.h / 2 -
			 * size) - 3 * size * 2, 0, 0); shot.angle = 90;
			 */
		...
		/*
		 * if (key == 6) { int size = MapGeneration.SIZE_FIGHT_FIGHT; final int
		 * speed_blade = size;
		 * 
		 * // final Square square = new Square(player.position.x + size * 2 + //
		 ...
		 * } }); mainTimer.start(); }
		 * 
		 * if (key == 7) { final int size = (int) (MapGeneration.SIZE_FIGHT);
		 * int s = MapGeneration.SIZE_FIGHT;
		 * 
		 * final Shot shot = new Shot(player.position, s, s, player.side *
		 ...
		 * } }); mainTimer.start(); }
		 * 
		 * if (key == 8) { final int size = (int) (MapGeneration.SIZE_FIGHT);
		 * int s = MapGeneration.SIZE_FIGHT;
		 * 
		 * final Shot shot = new Shot(player.position, s, s, player.side *
		 * (player.position.w / 2 - size / 2), (player.position.h / 2 - size) -
		 ...
		 * } }); mainTimer.start(); }
		 * 
		 * if (key == 9) { int size = MapGeneration.SIZE_FIGHT_FIGHT; final int
		 * speed_blade = size / 4; int w = player.side == 1 ? player.position.w
		 * : -size;
		 * 
		 * // final Square square = new Square(player.position.x + size * 2 + //
		 ...
		 * } }); mainTimer.start(); }
		 * 
		 * if (key == 10) { final int size = (int) (MapGeneration.SIZE_FIGHT);
		 * int s = MapGeneration.SIZE_FIGHT; final int speed_blade = size / 4;
		 * int w = player.side == 1 ? player.position.w : 0; final Shot shot =
		 * new Shot(player.position, s, s, player.side * (w / 2 - size / 2 + 2 *
